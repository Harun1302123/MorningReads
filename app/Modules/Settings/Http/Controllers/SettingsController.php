...
//ob#code@start - (galib) replacing values without any conditions
            $jsonData[] = [
                'caption' => 'Status',
...
//                    'sub_department.name as sub_department_name',
    ...
    public function createForcefullyDataUpdate()
    {
        ...
        try {
            ...
            return view("Settings::forcefully_data_update.create", compact('users'));
        } catch (\Exception $e) {
    ...
    public function storeForcefullyDataUpdate(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->get('column_name') as $key => $value) {
                ...
                $is_column_exists = DB::select("SELECT column_name FROM information_schema.columns WHERE table_name = '" . $table_name . "' AND column_name = '" . $column_name . "' LIMIT 1");
