...
//ob#code@start - (galib) can be less code and optimized
...
//ob#code@end - (galib)
            ...
            $organization->nid = $request->get('company_ceo_nid')??null;
            $organization->passport = $request->get('company_ceo_passport')??null;
            ...
            if (!empty($request->correspondent_signature_base64)) {
                ...
                $organization->entrepreneur_signature = correspondentPhoto($splited[1], $path, 'BSCIC_CP-sign-');
            }
    ...
    public function updateCompanyDirectorInfo(Request $request)
    {
        ...
        try {
            ...
            foreach ($inverstorInfo as $row) {
                ...
                $organization->nationality_type = ($row['nationality_type'] == "bangladeshi")?'bangladeshi':'foreign';
