...
//        window.opener.jQuery('#' + id + ' option:[value="' + val + '"]').replaceWith("<option value='" + val + "' selected>" + text + "</option>");
    ...
    window.opener.jQuery("select").removeAttr("multiple");

    window.opener.jQuery(".select2").css({
        "display": "none"
    });

    window.opener.jQuery(".selected-flag").css({
        "display": "none"
    });

    // Assuming you have a select element with the id "mySelect"
    window.opener.jQuery(document).ready(function() {
        window.opener.jQuery("select").select2();

        // Custom styling for Select2 dropdown
        window.opener.jQuery("select").on("select2:open", function () {
            // Customize the appearance of the opened dropdown here
            window.opener.jQuery(".select2-dropdown").css({
                "border": "none",
                "background": "#fff",
                "box-shadow": "none",
            });
    ...
    jQuery('#previewDiv').find('input:not([type=radio][type=hidden][type=file][name=acceptTerms]), textarea').each(function () {
        ...
        if (allClass && allClass.match("onlyNumber")) {
            if (allClass.match("nocomma")) {
                thisVal = this.value;
            } else {
                // thisVal = commaSeparateNumber(this.value);
                thisVal = this.value;
            }
        }

         if (this.type === 'checkbox') {
            // Check if it's a checkbox input with a value of 1
            if (thisVal === '1') {
                var checkboxClone = jQuery('<input type="checkbox" checked />').attr({
                    'name': this.name,
                    'value': this.value
                }).prop('disabled', true);
                jQuery(this).replaceWith(checkboxClone);
            } else {
                jQuery(this).replaceWith('<span style="line-height: 30px">' + thisVal + '</span>');
            }
        } else {
            // For inputs other than radio and checkbox
            jQuery(this).replaceWith('<span style="line-height: 30px">' + thisVal + '</span>');
        }
    });
