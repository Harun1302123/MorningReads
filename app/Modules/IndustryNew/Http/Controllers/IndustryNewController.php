...
class IndustryNewController extends Controller
{
    ...
    protected $process_type_id = 1;
    protected $acl_name = 'IndustryNew';
    ...
    public function appForm(Request $request)
    {
        ...
        try {
            $data = $this->getCommonDataForForm();
            ...
            //ob#code@start - (galib) define valid error code instead of [VA-1014]
    ...
    public function appStore(Request $request)
    {
        ...
        try {
            ...
            //ob#code@start - (galib) we can use $request->company_name_english format
...
//ob#code@start - (galib) can be separated
    ...
    public function appFormEdit($applicationId, $openMode = '', Request $request)
    {
        ...
        try {
            ...
            $data['companyUserType'] = CommonFunction::getCompanyUserType();
            $data['process_type_id'] = $this->process_type_id;
            $process_list_id = $data['appInfo']->process_list_id;
            $status_id = $data['appInfo']->status_id;
            $data['remarks_attachment'] = DB::select("select * from
                            `process_documents`
                            where `process_type_id` = $this->process_type_id and `ref_id` = $process_list_id and `status_id` = $status_id
                            and `process_hist_id` = (SELECT MAX(process_hist_id) FROM process_documents WHERE ref_id=$process_list_id AND process_type_id=$this->process_type_id AND status_id=$status_id)
                            ORDER BY id ASC"
);
            ...
            return response()->json(['responseCode' => 1, 'html' => CommonFunction::showErrorPublic($e->getMessage() . $e->getLine() . $e->getFile()) .  ' [VA-1015]']);
        }
    }
    ...
    {
        //ob#code@start - (galib) $payment_step_id not used
    ...
    //ob#code@start - (galib) unused method
...
//ob#code@end - (galib)
    ...
    {
        return Area::where('area_type', 1)->orderBy('area_nm_ban')->pluck('area_nm_ban', 'area_id')->toArray();
    }

    public function getDistricts()
    {
        return Area::where('area_type', 2)->orderBy('area_nm', 'ASC')->pluck('area_nm_ban', 'area_id')->toArray();
    }

    public function getThanas()
    {
        return Area::where('area_type', 3)->orderBy('area_nm', 'ASC')->pluck('area_nm_ban', 'area_id')->toArray();
    }

    public function getCommonDataForForm(): array
    {
        $data['divisions'] = $this->getDivisions();
        $data['districts'] = $this->getDistricts();
        $data['thanas'] = $this->getThanas();
