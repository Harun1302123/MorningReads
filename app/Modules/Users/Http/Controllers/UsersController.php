...
{
    //ob#code@start - Harun 
     //This class 45 methods, Maximum methods in a class should be 15-20 
     //Use Service class for logical work
     //Specify method return type 
    //ob#code@end - Harun
    ...
    public function assignParameters($id)
    {
        ...
        try {
            ...
            $user_info = Users::where('id', $user_id)->first(['id', 'desk_id', 'office_ids', 'user_email', 'user_mobile', 'user_first_name', 'user_middle_name', 'user_last_name', 'desk_training_id']);
    ...
    public function assignParametersSave(Request $request)
    {
        ...
        try {
            ...
            $deskData->office_ids = !empty($assign_park) ? $assign_park : 0;
            $deskData->save();
    ...
    public function failedLoginHist(Request $request, $email)
    {
        ...
        $decodedUserEmail = Encryption::decode($email);
...
//    }
    ...
    public function getAccessLogDataForSelf()
    {
        ...
        return Datatables::of($user_logs)->make(true);
    }
    ...
    public function getRowFailedData(Request $request)
    {
        try {
            $email = Encryption::decode($request->get('email'));
            ...
            return Datatables::of($failed_login_history)
                ...
                ->editColumn('remote_address', function ($failed_login_history) {
                    return $failed_login_history->remote_address;
                })
                ->removeColumn('id', 'is_sub_admin')
                ->make(true);
        } catch (\Exception $e) {
    ...
    public function storeNewUser(CreateUserRequest $request)
    {
        ...
        try {
            ...
            if (!empty($request->user_signature_base64)) {
                ...
                $user->signature = correspondentPhoto($splited[1], $path, 'signature_');
            }
            ...
            $user->desk_training_id = empty($request->get('training_assign')) ? null : $request->get('training_assign');
            $user->save();
    ...
    public function getThanaByDistrictId(Request $request)
    {
        try {
            ...
            $thanas = Area::select(DB::raw("CONCAT(AREA_ID,' ') AS AREA_ID"), 'area_nm_ban')
                ->where('PARE_ID', $district_id)->orderBy('area_nm_ban', 'ASC')->pluck('area_nm_ban', 'AREA_ID')->toArray();
    ...
    public function activate($id)
    {
        ...
        try {
            ...
            if ($user_active_status == 'active') {
                ...
                Session::flash('success', "User's Profile has been deactivated Successfully!");
            } else {
    ...
    function createdUserVerification($encrypted_token, Request $request)
    {
        try {
            $user = Users::where('user_hash', $encrypted_token)->first();
    ...
    public function profile_update(Request $request)
    {
        ...
        try {
            ...
            if (!empty($request->user_pic_base64)) {
                $yearMonth = date("Y") . "/" . date("m") . "/";
                $path = $yearMonth;
                $path_with_dir = 'users/profile-pic/' . $path;
                if (!file_exists('users/profile-pic/' . $path)) {
    ...
    public function resendVerificationFromAdmin()
    {
        if (!ACL::getAccsessRight('user', '-E-')) {
            abort('400', 'You have no access right! This incidence will be reported. Contact with system admin for more information.');
        }
