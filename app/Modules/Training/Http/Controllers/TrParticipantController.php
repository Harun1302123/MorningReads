...
class TrParticipantController extends Controller
{
    ...
    public function enrollParticipantsWithoutPayment(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'dob' => 'required',
            'profession' => 'required',
            'validate_field_attachment' => 'mime:pdf,doc,docx,jpg,jpeg,png|max:2048',
        ]);
        ...
        try{
            ...
            $trParticipant->course_id = $trSchedule->course_id;
            $trParticipant->user_id = Auth::user()->id;
            $trParticipant->full_name = $request->name;
            $trParticipant->dob = $request->dob;
            $trParticipant->profession = $request->profession;
            if($request->profession == 'student'){
                $trParticipant->ins_name = $request->ins_name;
                $trParticipant->last_qualification = $request->last_qualification;
            }else{
                $trParticipant->organization_name = $request->org_name;
                $trParticipant->designation = $request->designation;
            }
            $trParticipant->is_paid = 1;
            $trParticipant->image_path = $request->applicant_photo ? $request->applicant_photo : '';
            $trParticipant->attachment_type = $request->attachment_type ? $request->attachment_type : '';
            ...
            $applicantEmailPhone =  Users::query()
                ...
                ->get(['user_email', 'user_mobile as user_phone']);
    ...
    public function enrollParticipants(Request $request, $id)
    {
        ...
        try {
            ...
            $appData = TrPaymentMaster::firstOrNew([
                'tr_session_id' => $trSessionId,
            ...
            ->addColumn('action', function ($list) {
                if (in_array(Auth::user()->user_type, ['4x404', '1x101']) && (ACL::getAccsessRight('Training-Desk', '-E-') || ACL::getAccsessRight('Training-Desk', '-V-'))) {
                    ...
                    if ($list->status == 'Declined') {
                        ...
                        $html = '<a href="javascript:void(0)" class="btn-sm btn btn-danger actionActivates" data-id="' . Encryption::encodeId($list->id) . ' " data-action="Decline"> Decline </a>';
                    }
                } else {
    ...
    public function courseDetails($id)
    {
        ...
        $course = TrSchedule::find($decodeId);
        ...
        if ($participantinfo) {
            ...
            $is_evaluated = TrEvaluation::where('participant_id', $participant)
                ->where('schedule_id', $decodeId)
                ->where('batch_id', $course->batch_id)
                ->where('evaluation_type', 'Final Evaluation')
                ->exists();
        }
