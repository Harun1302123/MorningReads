...
class CompanyAssociationController extends Controller
{
    ...
    public function __construct()
    {
        ...
        //ob#code@start - (galib) can be declared statically, no need to be in constructor
    ...
    {
        //ob#code@start - (galib) (*1) we can show the abort message from ACL.php itself in else condition. And the message can be declared statically in a variable in ACL.php
...
//ob#code@start - (galib) can be done before db transaction
...
//ob#code@end - (galib)
        ...
        //ob#code@start - (galib) can be separated from db transaction
        if (!empty($request->get('user_type')) && $request->get('user_type') == 'company') {
            $appInfo = [
    ...
    public function companyActivatesAction(Request $request)
    {
        ...
        try {
            ...
            //ob#code@start - (galib) could use switch here
            if ($key == 'approved') {
    ...
    public function associationCompanyInfo(Request $request)
    {
        ...
        } elseif ($request->get('companyType') == 1
