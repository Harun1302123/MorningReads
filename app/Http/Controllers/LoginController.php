...
//        $otpBy = trim($request->get('otp'));
            ...
            if ($email) {
                ...
                if ($user) {
                   ...
                   $data = ['responseCode' => 1, 'msg' => 'Valid email', 'user_email'=>$email, 'queue_id'=> Encryption::encodeId($id)];
                    ...
                    $data = ['responseCode' => 0, 'msg' => 'This user is not present in system or Inactive user. Contact with system admin.'];
                    return response()->json($data);
                }
            } else {
    ...
    */
    ...
    public function otpLoginMobileValidationWithTokenProvide(Request $request)
    {
        try {


            $user_mobile = trim($request->get('email_address'));

            /*
             * User given data is OK
             */
            if ($user_mobile) {
                $user = Users::where('user_mobile', $user_mobile)
                    ->where('is_approved', 1)
                    ->where('user_status', 'active')->first();
                ...
                if ($user) {
                   ...
                   $id =  CommonFunction::sendEmailSMS('ONE_TIME_PASSWORD', $appInfo, $receiverInfo);
                   $data = ['responseCode' => 1, 'msg' => 'Valid mobile number', 'user_mobile'=>$user_mobile, 'queue_id'=> Encryption::encodeId($id)];
                    return response()->json($data);
                } else {
                    $data = ['responseCode' => 0, 'msg' => 'This user is not present in system or Inactive user. Contact with system admin.'];
                    return response()->json($data);
                ...
                $data = ['responseCode' => 0, 'msg' => 'Invalid mobile number'];
                return response()->json($data);
            }
        } catch (\Exception $exception) {
            $data = ['responseCode' => 0, 'msg' => 'Sorry! Something is Wrong.' . $exception->getMessage()];
    ...
    private function commonLoginCheck($request, $usersModel, $loginType = 0, $otp = '', $is_ajax_request = false)
    {
        try {
            ...
            if ($loginType == 1) {
                ...
                $user = $usersModel::where('user_mobile', $request->get('email_address'))->where('login_token', $request->login_token)->first();
...
//                                return \redirect()->to('company-association/select-company');
    ...
    public function _checkUserTypeRootActivation($userType, $is_ajax_request)
    {
