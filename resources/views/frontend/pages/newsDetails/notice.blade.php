    ...
    <main class="site-main-content page-height home-hd-video-sec container mb-5">
        <section class="privacy-policy-section">
            ...
            <div class="notice container mb-4">
                ...
                <div class="container">
                    @foreach ($notice as $single_notice)
                        <div class="notification-ui_dd-content">
                            <div class="notification-list notification-list--unread d-flex justify-content-between align-items-center p-3 border rounded mb-3">
                                <div class="notification-list_content col-md-8">
                                    <div class="notification-list_detail">
                                        <h5>{{ $single_notice->title_en }}</h5>
                                        <p>{{ Illuminate\Support\Str::limit($single_notice->content_en, $limit = 250, $end = '...') }}</p>
                                        <p class="date"><small><i class="far fa-calendar text-muted"></i> {{ !empty($single_notice->updated_at) ? date('d M Y', strtotime($single_notice->updated_at)) : null }}</small></p>
                                    </div>
                                </div>
                                <div class="details-link d-flex">
                                    <a href="{{ route('frontend.single_notice_details', ['id' => Encryption::encodeId($single_notice->id)]) }}" class="text-decoration-none">
                                        <p class="news-details-link mt-2 text-success font-weight-bold d-flex align-items-center">
