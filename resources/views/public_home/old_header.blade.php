...
<nav class="navbar navbar-expand-lg navbar-light navbar-laravel">
    <div class="container">
        ...
        <a class="navbar-brand" href="{{ config('app.PROJECT_ROOT') }}">
            {{-- <img style="width: 70%" src="{{ asset(Cache::get('logo-info')->logo) }}" alt="{{ config('app.project_name') }} - Logo" class="img-responsive"> --}}
            <img style="width: 25%" src="{{ url(Cache::get('logo-info')->logo) }}"
            ...
            <ul class="navbar-nav ml-auto">
                @if (request()->route() == null ||
                    ...
                    <li class="active menu-text nav-item"><a class="nav-link"
                                                             href="{{ url('/') }}">{!! trans('messages.header.home_page') !!}</a>
                    </li>
                @endif
