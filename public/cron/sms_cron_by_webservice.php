...
if ($get_pending_sms = $mysqli->query("SELECT id, sms_content, sms_to, sms_no_of_try FROM email_queue
    ...
    while ($sms_data = $get_pending_sms->fetch_assoc()) {
        ...
        $sms_api_url = config('app.SMS_API_URL_FOR_SEND', 'https://api-k8s.oss.net.bd/api/broker-service/sms/send_sms');
        ...
        $status_update_query = "UPDATE email_queue SET sms_status=$sms_status,sms_response_id='$sms_response_id', sms_response='" . $response . "', sms_no_of_try=$sms_no_of_try WHERE id=$id";
        $mysqli->query($status_update_query);
    }
}
...
//    }
    ...
    $access_token_url = config('app.SMS_API_URL_FOR_TOKEN', 'https://idp.oss.net.bd/auth/realms/dev/protocol/openid-connect/token');
    $access_data = [
        "client_id" => config('app.SMS_CLIENT_ID', 'bisic-oss-client'),
        "client_secret" => config('app.SMS_CLIENT_SECRET', '147cff25-f852-476a-b40c-e2270bb3a9df'),
        "grant_type" => config('app.SMS_GRANT_TYPE', 'client_credentials')
    ];
    try {
        $curl_handle = curl_init();
