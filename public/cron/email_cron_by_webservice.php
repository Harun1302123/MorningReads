...
if ($get_pending_email = $mysqli->query("SELECT id,app_id,caption,email_to,email_cc,email_content,email_no_of_try,attachment,email_subject,attachment_certificate_name 
    ...
    while ($row = $get_pending_email->fetch_assoc()) {
        ...
        $sms_api_url_for_token = config('app.EMAIL_API_URL_FOR_SEND', 'https://api-k8s.oss.net.bd/api/broker-service/email/send_email');
        $base_email_for_api = config('app.EMAIL_FROM_FOR_EMAIL_API', 'onestopservice@bscic.gov.bd');
        ...
        $status_update_query = "UPDATE email_queue SET email_status=$email_status,email_response_id='$email_response_id',email_response='" . $result . "', email_no_of_try=$email_no_of_try WHERE id=$id";
        $mysqli->query($status_update_query);
    }
}
...
//    }
    ...
    $sms_api_url_for_token = config('app.SMS_API_URL_FOR_TOKEN', 'https://idp.oss.net.bd/auth/realms/dev/protocol/openid-connect/token');
    $sms_client_id = config('app.SMS_CLIENT_ID', 'bisic-oss-client');
    $sms_client_secret = config('app.SMS_CLIENT_SECRET', '147cff25-f852-476a-b40c-e2270bb3a9df');
    $sms_grant_type = config('app.SMS_GRANT_TYPE', 'client_credentials');
